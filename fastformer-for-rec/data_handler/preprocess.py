...
class NewsInfo:
    ...
    def sent_tokenize(self, sent, max_len):
        ...
        # HF >= 4.x không còn  → dùng padding="max_length"
...
def get_doc_input(news, news_index, category_dict,
    ...
    for key in tqdm(news):
        ...
        if 'title' in args.news_attributes:
            max_len = args.num_words_title
            ids = _fix_len(title['input_ids'], max_len)
            att = _fix_len(title['attention_mask'], max_len)
            news_title[doc_index] = ids
            news_title_attmask[doc_index] = att

        if 'abstract' in args.news_attributes:
            max_len = args.num_words_abstract
            ids = _fix_len(abstract['input_ids'], max_len)
            att = _fix_len(abstract['attention_mask'], max_len)
            news_abstract[doc_index] = ids
            news_abstract_attmask[doc_index] = att

        if 'body' in args.news_attributes:
            max_len = args.num_words_body
            ids = _fix_len(body['input_ids'], max_len)
            att = _fix_len(body['attention_mask'], max_len)
            news_body[doc_index] = ids
            news_body_attmask[doc_index] = att
